...
**约定**：CLI 路径用绝对路径 | 文件命名 `script_<主题>_<日期>.json` | 默认目录 `assets/`（中间）/ `output/`（最终）/ `assets/images/`（图片）

**断点续传**：用户中断时保留当前阶段产物，下次从断点 Phase 继续（检查 `assets/` 已有文件判断进度）。
...
- 自定义风格：`clawreel music --prompt "风格描述" --duration 90 --output /abs/path/bg_music_topic.mp3`
...
CLI 输出 JSON 到 stdout，**用 Write 工具保存为** `assets/script_<主题>_<日期>.json`。
...
python3 -c "
import json
segs = json.load(open('assets/segments_<主题>_<日期>.json'))
script = json.load(open('assets/script_<主题>_<日期>.json'))
errors = []
...
图片保存到 `assets/images/seg_*.jpg`。`--video` 同时生成 6 秒片头视频。
...
ffmpeg -y \
  ...
  -t <时长> /abs/path/output/composed.mp4
```
