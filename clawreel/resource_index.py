...
class ResourceManifest:
    """资源清单"""
    version: str = "1.0"
    topics: Dict[str, dict] = field(default_factory=dict)  # topic -> {keywords, resources}

    def save(self, path: Path):
        with open(path, 'w', encoding='utf-8') as f:
            json.dump({"version": self.version, "topics": self.topics}, f, ensure_ascii=False, indent=2)
...
CHINESE_STOPWORDS = {
    ...
    '自己', '这', '那', '么', '它', '什么', '怎么', '如何', '为什么', '怎么样',
    '关于', '对于', '这个', '那个', '可以', '能', '应该', '需要', '可能', '一定',
    '最', '更', '非常', '特别', '真的', '其实', '当然', '而且', '或者', '以及',
    '因为', '所以', '如果', '虽然', '但是', '然后', '还是', '以及', '并',
    '之', '与', '及', '等', '种', '个', '些', '点', '方面', '问题', '情况'
}
...
class KeywordExtractor:
    ...
    COMMON_WORDS = {
        '未来', '趋势', '发展', '变化', '影响', '重要', '关键', '机会', '挑战',
        '技术', '创新', '改变', '行业', '市场', '社会', '生活', '工作', '学习',
        '健康', '财富', '投资', '创业', '科技', '互联网', '数字化', '智能化',
        '人工智能', '机器学习', '大数据', '云计算', '区块链', '元宇宙', '虚拟现实',
        '短视频', '直播', '电商', '网红', '品牌', '营销', '运营', '内容创作',
    }
    ...
    def extract(self, text: str, top_k: int = 10) -> List[str]:
        ...
        # 3. 过滤停用词和单字
        words = [w for w in chinese_words
                if len(w) >= 2 and w not in CHINESE_STOPWORDS]
...
class SimilarityCalculator:
    ...
    def calculate_tfidf_similarity(self, keywords1: List[str], keywords2: List[str]) -> float:
        """TF-IDF 相似度（简化版，使用词频）"""
        if not keywords1 or not keywords2:
            return 0.0
...
class ResourceIndexer:
    ...
    def register_resource(self, resource_type: str, file_path: str, topic: str):
        ...
        created = datetime.fromtimestamp(path.stat().st_ctime).isoformat() if path.exists() else ""

        # 更新清单
        if topic not in self.manifest.topics:
            self.manifest.topics[topic] = {
                "keywords": keywords,
                "resources": []
            }
    ...
    def find_similar(self, topic: str, threshold: float = 0.3) -> List[SimilarityResult]:
        """查找相似主题的资源"""
        results = []
...
class SmartSuggestionGenerator:
    ...
    def generate_suggestions(self, topic: str, needed_types: List[str] = None) -> dict:
        """生成资源复用建议
        ...
        # 2. 语义相似搜索
        similar = self.indexer.find_similar(topic, threshold=self.SIMILARITY_THRESHOLDS["low"])
    ...
    def _generate_action_suggestions(self, result: dict) -> List[str]:
        ...
        if result["exact_match"]:
            ...
            high_conf = [m for m in result["similar_matches"] if m["confidence"] == "high"]
            if high_conf:
                suggestions.append(f"🎯 发现 {len(high_conf)} 个高度相似的资源，可考虑复用")
